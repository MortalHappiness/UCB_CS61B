...
public class HugLife {
  ...
  /** Size of the world. Probably best to keep this under 100 or so. */
  ...
  /** Time in milliseconds between simulation steps. Reduce to make things run faster. */
  public static final int PAUSE_TIME_PER_SIMSTEP = 100;

  /**
   * If true, the HugLifeAnimator class saves an image after every cycle. After all cycles have
   * finished, or on exit, these images are combined into an animated GIF. Requires ImageMagick and
   * only works in SIMULATE_BY_CYCLE mode. Consider reducing PAUSE_TIME_PER_SIMSTEP when this flag
   * is on, since file writes will take some time. See the HugLifeAnimator class for details.
   */
  public static final boolean GENERATE_GIF = false;

  /**
   * Name of output GIF (relative to working directory), assuming GENERATE_GIF is true. Should end
   * in ".gif".
   */
  ...
  /**
   * A set of precanned hard-coded worlds. This is terrible style, but hey it's very easy to write
   * this way.
   */
  ...
  /**
   * Reads the world from file with worldName and intialized a HugLife with the contents of the file
  ...
  /**
   * By default, the simulator simulates by cycle, i.e. allows every creature to move before
   * drawing. If you set this to false, then the world will be drawn between moves (much slower).
   */
